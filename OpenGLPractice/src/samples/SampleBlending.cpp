...
namespace sample
{
		...
		: m_ClearColor{ 0.2f, 0.3f, 0.3f, 1.0f }
	{
		...
		m_TextureTransparent = std::make_unique<Renderer::Texture>("res/textures/grass.png");
	...
	void SampleBlending::OnUpdate(float deltaTime)
	{

	}

	void SampleBlending::OnRender(const Camera& camera, RenderScene* scenebuffer)
	{
		...
		for (unsigned int i = 0; i < vegetation.size(); ++i)
		{
			...
			model = glm::rotate_slow(model, glm::radians(-180.0f), camera.GetFront());
			m_Shader->SetUniformMat4f("model", 1, model);
			glDrawArrays(GL_TRIANGLES, 0, 6);
		}
	}

	void SampleBlending::OnImGuiRenderer()
	{
	...
	int SampleBlending::m_Width = 800;

	int SampleBlending::m_Height = 600;
