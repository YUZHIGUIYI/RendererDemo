...
namespace Renderer
{
	...
	public:
		...
		// ------------------------------------------------------------------------
		ShaderN(const char* vertexPath, const char* fragmentPath, const char* geometryPath = nullptr,
			const char* tessControlPath = nullptr, const char* tessEvalPath = nullptr)
		{
			...
			{
				RD_CORE_ERROR("ERROR::SHADER::FILE_NOT_SUCCESFULLY_READ");
			}
		...
		void checkCompileErrors(GLuint shader, std::string type)
		{
			...
			if (type != "PROGRAM")
			{
				...
				if (!success)
				{
					...
					RD_CORE_ERROR("ERROR::SHADER_COMPILATION_ERROR of type: {0} {1}", type, infoLog);
				}
			}
			else
			{
				...
				if (!success)
				{
					...
					RD_CORE_ERROR("ERROR::PROGRAM_LINKING_ERROR of type: {0} {1}", type, infoLog);
				}
			}
		}
	};
