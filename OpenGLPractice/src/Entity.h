...
struct Plan
{
	...
	Plan(const glm::vec3& pl, const glm::vec3& norm)
		: normal(glm::normalize(norm)), distance(glm::dot(normal, pl)) 
	{}
...
struct SquareAABB : public BoundingVolume
{
	...
	bool isOnOrForwardPlan(const Plan& plan) const final
	{
		// Compare the projection interval radius of b onto L(t) = b.c + t * p.n
...
Frustum createFrustumCamera(const Camera& cam, float aspect, float fovY, float zNear, float zFar)
{
	Frustum frustum;
...
public:
	...
	{
		// Get global scale thanks to our transform
		const glm::vec3 globalCenter{ transform.getModelMatrix() * glm::vec4(boundingVolume->center, 1.0f) };
	...
	void drawSelfAndChild(const Frustum& frustum, Renderer::Shader& ourShader, unsigned int& display,
		unsigned int& total)
	{
		if (boundingVolume->isOnFrustum(frustum, transform))
		{
			ourShader.SetUniformMat4f("model", 1, transform.getModelMatrix());
