...
bool BloomFBO::Init(unsigned int windowWidth, unsigned int windowHeight, unsigned int mipChainLength)
{
	...
	{
		RD_ERROR("Window size conversion overflow - cannot build bloom FBO!");
	}
	...
	for (unsigned int i = 0; i < mipChainLength; ++i)
	{
		...
		RD_INFO("Created Bloom Mip {0} x {1}", mipIntSize.x, mipIntSize.y);
	...
	{
		RD_ERROR("GBuffer FBO Error, status: {0}", status);
